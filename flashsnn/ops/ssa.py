...
def _ssa_backward_kernel_without_atomic(
    ...
    """If atomic_add is not available, we loop on the sequence dimension of Q."""
    tn = tl.program_id(0)
    head = tl.program_id(1)
    ...
    grad_k_buffer = tl.zeros((BLOCK_Cph, BLOCK_LK), dtype=dtype)
    grad_v_buffer = tl.zeros((BLOCK_LK, BLOCK_Cph), dtype=dtype)

    scale = tl.full((1,), scale, dtype=dtype)

    for lq_start in tl.static_range(0, L, BLOCK_LQ):  # sequentially process Q
        q_ptrs = tl.make_block_ptr(
            q_base,
            shape=(Cph, L),
            strides=(L, 1),
        ...
        for lk_start in tl.static_range(0, L, BLOCK_LK):
            ...
            grad_k_buffer += grad_k
            grad_v_buffer += grad_v
    ...
    tl.store(grad_k_ptrs, grad_k_buffer * scale, boundary_check=(0, 1))
    ...
    tl.store(grad_v_ptrs, grad_v_buffer * scale, boundary_check=(0, 1))
